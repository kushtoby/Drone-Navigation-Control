...
class PinkBalloonDetector(Node):
    def __init__(self) -> None:
        ...
        # Keep your current pink HSV thresholds
        self.lower_pink = np.array([140, 80, 80], dtype=np.uint8)
        self.upper_pink = np.array([179, 255, 255], dtype=np.uint8)

        # New main trigger: total pink pixels in mask
        self.min_pixel_count = 320

        # Still keep a small blob threshold for choosing a target blob
        self.min_blob_area = 120

        # Ignore lower part of frame where floor clutter often appears
        self.roi_top_frac = 0.0
        self.roi_bottom_frac = 0.9
    ...
    def image_callback(self, msg: Image) -> None:
        ...
        try:
            ...
            # Primary trigger: how many pink pixels exist at all
            ...
            for cnt in contours:
                ...
                # Prefer larger blobs closer to image center
            ...
            self.detected_pub.publish(detected_msg)
            self.get_logger().error(f'Pink balloon detection failed: {e}')
...
def main(args=None) -> None:
    ...
    try:
        ...
        node.get_logger().info('Shutting down pink balloon detector.')
    finally:
        node.destroy_node()
        rclpy.shutdown()
# import rclpy
...
#         # Pink HSV range.
#         # This is a good starting range for a bright pink balloon indoors.
#         self.lower_pink = np.array([140, 80, 80], dtype=np.uint8)
#         self.upper_pink = np.array([179, 255, 255], dtype=np.uint8)

#         # Start conservative, then tune after checking real balloon area at distance.
#         self.min_area = 300
...
#                 dist = ((cx - img_cx) ** 2 + (cy - img_cy) ** 2) ** 0.5
#                 score = area + 1500.0 * circularity - 2.0 * dist
...
#             else:
#                 self.get_logger().info(
#                     f'Pink balloon not confirmed. raw_detected={raw_detected}, history={list(self.history)}'
#                 )
